...
class AttendController extends AppController{
	...
	function ifEvent(){
		...
		$eventName = $this->Event->findEventName('2012-12-31');
	...
	function updateRecord(){
		...
		if(!empty($this->data['Reserv']['am_id'])){
			$saveReserv['id'] = $this->data['Reserv']['am_id'];
			$this->Reserv->save($saveReserv);
		}
		if(!empty($this->data['Reserv']['pm_id'])){
			$saveReserv['id'] = $this->data['Reserv']['pm_id'];
			$this->Reserv->save($saveReserv);
		}
		$this->redirect('.');
	}
