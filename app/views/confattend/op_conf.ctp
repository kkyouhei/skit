...
<body>
	<div id="contents">
		<p>OPの検索結果を表示する画面です confattend/opConf.ctp</p>
		...
		<div id="form">
			...
			for($i=0 ; $i<count($sankashas) ; $i++){
				if($fieldId != $sankashas[$i]['Field']['field_name'] || $eventTime != $sankashas[$i]['Event']['event_time']){
					echo $sankashas[$i]['Field']['field_name'] . 
					     $sankashas[$i]['Event']['event_time'] .
						"<br />" ;
					echo "予約ID　参加者名　連絡先　メールアドレス　出席状況 <br />";
				}
				$fieldId = $sankashas[$i]['Field']['field_name'];
